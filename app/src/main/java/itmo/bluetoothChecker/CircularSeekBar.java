...
public class CircularSeekBar extends View {
    ...
    protected static final float DEFAULT_POINTER_HALO_WIDTH = 10f;
    ...
    protected static final boolean DEFAULT_USE_CUSTOM_RADIUS = false;
    ...
    protected static final String DEFAULT_AD = "120/70";
    /**
    ...
    */
    ...
    protected Rect mTitleBoundRect;

    protected Paint titlePaint;

    protected Rect mTextBoundRect;
    ...
    protected void initAttributes(TypedArray attrArray) {
        ...
        mCustomRadius = attrArray.getBoolean(R.styleable.CircularSeekBar_use_custom_radius, DEFAULT_USE_CUSTOM_RADIUS);
        ...
        mValue = attrArray.getString(R.styleable.CircularSeekBar_value);
        mHint = attrArray.getString(R.styleable.CircularSeekBar_hint);
        mIcon = attrArray.getResourceId(R.styleable.CircularSeekBar_drawableSource, R.drawable.ic_icon_rate);
        mAD = attrArray.getBoolean(R.styleable.CircularSeekBar_AD, false);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mAD) {
            mValue = String.valueOf(getResources().getStringArray(R.array.pressure)[mProgress * 2]);
        }else{
            //mValue = String.valueOf(mProgress );
            mValue = String.valueOf(getResources().getStringArray(R.array.number)[mProgress + mMin + 1]);
        }
        ...
        Bitmap bitmap = getBitmapFromDrawable(getContext(), mIcon);
        canvas.drawBitmap(bitmap, mCircleRectF.centerX() - (bitmap.getWidth() >> 1),
                mCircleRectF.centerY() - (bitmap.getHeight() / 1.5f), drawPaint);

        /*
        canvas.save();
        Drawable mMyVectorDrawable = VectorDrawableCompat.create(getResources(), R.drawable.ic_bleed, null);
        Objects.requireNonNull(mMyVectorDrawable).setBounds(-30, -30, 20 , 20);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mMaintainEqualCircle) {
            ...
            setMeasuredDimension(min, min + 30);
        } else {
        ...
        if (mCustomRadius) {
            // Check to make sure the custom radius are not out of the view. If they are, just use the view values
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float x = event.getX() - (getWidth() >> 1);
        float y = event.getY() - (getHeight() >> 1);
